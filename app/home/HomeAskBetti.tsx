...
export function resolveAskBettiDestination(value:string){
 ...
 if(/\b(receipt)\b/.test(lower)){const merchant=text.replace(/\b(did|do|i|have|upload|uploaded|find|show|me|my|the|a|an|receipt|for|this|purchase)\b/gi,' ').replace(/[?!.]+$/g,'').replace(/\s+/g,' ').trim();return merchant?`/transactions?q=${encodeURIComponent(merchant)}`:'/receipts'}
 if(/\b(transaction|purchase|charge|fix|correct)\b/.test(lower))return'/transactions'
 return null
}

const suggestions=[['Show reports','/reports'],['Add mileage','/mileage'],['Recent transactions','/transactions'],['Upload a receipt','/receipts'],['Create an invoice','/invoices']]as const
...
export function HomeAskBetti(){const router=useRouter(),[value,setValue]=useState(''),[limited,setLimited]=useState(false)
 ...
 return <section className="home-ask" aria-labelledby="ask-betti-heading"><div className="home-ask-heading"><div><p className="home-kicker">Ask Betti</p><h2 id="ask-betti-heading">How can I help?</h2><p>Ask about a record or tell me what you want to do in WriteOffs.</p></div><BettiIllustration state="welcome" className="home-ask-betti" sizes="6rem" decorative/></div><form onSubmit={submit}><label className="sr-only" htmlFor="ask-betti-input">Ask Betti about your books</label><input id="ask-betti-input" value={value} onChange={event=>{setValue(event.target.value);setLimited(false)}} placeholder="Ask Betti about your books…"/><button className="btn btn-primary">Continue</button></form><nav className="home-ask-suggestions" aria-label="Things Betti can help you find">{suggestions.map(([label,href])=><Link key={href} href={href}>{label}</Link>)}</nav>{limited&&<div className="home-ask-limit" role="status"><p>I can help you find records and common WriteOffs tasks today, but I can’t safely answer that question from here yet.</p><div><Link href="/transactions">Find a transaction</Link><Link href="/reports">See reports</Link><Link href="/questions">See what I need</Link></div></div>}</section>}
