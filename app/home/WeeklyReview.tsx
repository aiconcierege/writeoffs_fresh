...
const stageCopy:Record<WeeklyReviewStage,{kicker:string;title:string}>={personal:{kicker:'First, the exceptions',title:'First, take a quick look at this week’s transactions.'},mixed:{kicker:'Next, shared purchases',title:'Did any of these include both business and personal expenses?'},questions:{kicker:'A few facts only you know',title:'Now I can finish the bookkeeping.'},documentation:{kicker:'Receipts and records',title:'Let’s finish the documentation check.'},mileage:{kicker:'Business driving',title:'What about mileage this week?'},final:{kicker:'Your cleaned books',title:'I’m preparing the final review.'}}

function TransactionRow({item,selectable,selectionPurpose,selected,onSelect,children}:{item:WeeklyReviewTransaction;selectable?:boolean;selectionPurpose?:'personal'|'mixed';selected?:boolean;onSelect?:()=>void;children?:React.ReactNode}){
 ...
 return <li className={`weekly-transaction-row${selected?' is-selected':''}`}><label>
  {selectable&&<input type="checkbox" checked={selected} onChange={onSelect} aria-label={selectionPurpose==='personal'?`Mark ${item.merchant} as personal`:`Mark ${item.merchant} as business and personal`}/>}<span className="weekly-transaction-main"><strong>{item.merchant}</strong><span className="weekly-transaction-meta"><time dateTime={item.date}>{formatReviewActivityDate(item.date)}</time>{category} · {treatment}</span><span className={`weekly-receipt-state ${item.hasReceipt?'has-receipt':'missing-receipt'}`}>{item.hasReceipt?'Receipt attached':'No receipt'}</span></span><b>{money.format(Math.abs(item.amountCents)/100)}</b>
...
function WorkflowReview({review,currentQuestionCount}:{review:CustomerWeeklyReview;currentQuestionCount:number}){
 ...
 return <section className="home-review weekly-workflow" aria-labelledby="weekly-review-heading"><header className="home-review-heading"><div><p className="home-kicker">Your weekly check-in</p><p className="home-review-period">{formatReviewPeriod(review.periodStart,review.periodEnd)}</p><h2 id="weekly-review-heading">{copy.title}</h2><p>{copy.kicker}</p></div></header>
  <div className="weekly-workflow-body">
   {stage==='personal'&&<><div className="weekly-sweep-intro"><p>Check anything that was personal. I’ll remove those before we continue.</p><p>You don’t need to confirm every business transaction.</p></div><SweepActions stage="personal" selectedCount={selected.size} busy={busy} onConfirm={()=>void advance()} onNone={()=>void advance()}/><ul className="weekly-transaction-list">{review.transactions.map(item=><TransactionRow key={item.id} item={item} selectable selectionPurpose="personal" selected={selected.has(item.id)} onSelect={()=>toggle(item.id)}/>)}</ul></>}
   {stage==='mixed'&&<><div className="weekly-sweep-intro"><p>Select anything that included both business and personal spending.</p><p>For each one, tell me the business dollars. I’ll handle the allocation.</p></div><SweepActions stage="mixed" selectedCount={selected.size} busy={busy} onConfirm={()=>void advance()} onNone={()=>void advance()}/><ul className="weekly-transaction-list">{visible.filter(item=>item.amountCents<0).map(item=><TransactionRow key={item.id} item={item} selectable selectionPurpose="mixed" selected={selected.has(item.id)} onSelect={()=>toggle(item.id)}>{selected.has(item.id)&&<label className="weekly-business-amount">How much of the {money.format(Math.abs(item.amountCents)/100)} was for your business?<span><span aria-hidden="true">$</span><input aria-label={`Business amount for ${item.merchant}`} inputMode="decimal" value={amounts[item.id]??''} onChange={event=>setAmounts(value=>({...value,[item.id]:event.target.value}))} placeholder="0.00"/></span></label>}</TransactionRow>)}</ul></>}
   ...
   {stage!=='personal'&&stage!=='mixed'&&<footer className="weekly-workflow-actions"><button className="btn btn-primary" disabled={busy||(stage==='questions'&&currentQuestionCount>0)||(stage==='documentation'&&missing.length>0)} onClick={()=>void advance()}>{busy?'Saving…':stage==='questions'?'Continue to receipts':stage==='documentation'?'Continue to mileage':stage==='mileage'?(review.mileage.entries.length?'No, that’s everything':'No, no business driving'):'Prepare my review'}</button><button className="btn btn-quiet">Not right now</button></footer>}
  </div></section>
}

function SweepActions({stage,selectedCount,busy,onConfirm,onNone}:{stage:'personal'|'mixed';selectedCount:number;busy:boolean;onConfirm:()=>void;onNone:()=>void}){
 const noun=stage==='personal'?'personal':'business + personal'
 return <div className="weekly-sweep-actions" aria-live="polite"><strong>{selectedCount} selected</strong><div><button type="button" className="btn btn-primary" disabled={busy||selectedCount===0} onClick={onConfirm}>{busy?'Saving…':stage==='personal'?'Mark as personal':'Continue with selected'}</button><button type="button" className="btn btn-quiet" disabled={busy||selectedCount>0} onClick={onNone}>None were {noun}</button></div></div>
}
