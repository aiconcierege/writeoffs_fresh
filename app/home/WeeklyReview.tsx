...
export function WeeklyReview({review}:{review:CustomerWeeklyReview}){
 ...
 return <section className="home-review" aria-labelledby="weekly-review-heading">
  ...
  <div className="home-review-sheet">
   ...
   <ul className="home-review-items">{review.items.slice(0,5).map(item=><li key={item.id}><Link href={item.transactionId?`/transactions/${item.transactionId}?review=${review.id}&snapshot=${review.snapshotId}&event=${review.eventId}`:'/receipts'}><span><strong>{item.label}</strong><small>{item.date} · {item.role==='income'?'Business income':item.treatment==='mixed_use'?'Business and personal':'Business expense'}</small></span><b>{money.format(Math.abs(item.amountCents)/100)}</b></Link></li>)}</ul>
