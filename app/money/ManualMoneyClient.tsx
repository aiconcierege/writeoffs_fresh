...
export function ManualMoneyClient({ initialActivities, initialDirection }: {
  ...
  return <main className="app-page"><div className="page-container page-container-narrow">
    <header><p className="text-xs font-semibold tracking-[0.16em] text-slate-500">MONEY OUTSIDE CONNECTED ACCOUNTS</p>
      <h1 className="page-title">Tell WriteOffs what happened</h1>
    ...
    <div className="mt-8 grid grid-cols-2 gap-2 rounded-xl bg-[#edf2ee] p-1.5" role="group" aria-label="Activity type">
    ...
    <section className="mt-12 border-t border-slate-200 pt-8"><h2 className="text-xl font-semibold text-slate-950">Recorded activity</h2>
      {activities.length === 0 ? <p className="mt-4 text-sm text-slate-600">Nothing recorded here yet.</p>
        : <div className="record-list mt-4">{activities.map((activity) => <article key={String(activity.manual_financial_source_id)} className="record-row py-5">
...
function MoneyForm({ direction, activity, onDone }: { direction: ManualMoneyDirection; activity: Activity | null; onDone: () => Promise<void> }) {
  ...
  return <form className="surface mt-7 p-5 sm:p-7" onSubmit={async(event)=>{event.preventDefault();setBusy(true);setError(null);const form=new FormData(event.currentTarget)
    ...
    {error?<p role="alert" className="notice notice-error mt-4">{error}</p>:null}<button disabled={busy} className="btn btn-primary mt-6 w-full sm:w-auto">{busy?'Saving…':activity?'Save correction':received?'Record money received':'Record money spent'}</button>
  </form>
}
