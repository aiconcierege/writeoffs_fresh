...
export function SpecialTransactionFlow({work,returnTo:origin,embedded=false,onResolved,onRecoveryRefresh,onOrdinaryRequest}:{work:SpecialWork;returnTo:string;embedded?:boolean;onResolved?:(deferred:boolean,message?:string,work?:GuidedWorkProjection)=>Promise<void>;onRecoveryRefresh?:()=>Promise<void>;onOrdinaryRequest?:()=>void}){
 ...
 return <section className={embedded?"betti-special space-y-4":"mx-auto max-w-2xl space-y-4 py-5"} aria-label="Betti transaction question">
  ...
  {work.kind==='refund'&&!work.linked&&!needsSupportingRecord&&<>{(!embedded||(!work.candidates.length&&!['merchant_return','reimbursement'].includes(work.lastAction??'')))&&<><h1 className="text-2xl font-semibold">Was this money returned by a store, or did someone reimburse you?</h1><div className="flex flex-wrap gap-3"><button className="btn btn-secondary" disabled={busy} onClick={()=>void answer('merchant_return')}>Returned by the store</button><button className="btn btn-secondary" disabled={busy} onClick={()=>void answer('reimbursement')}>Someone reimbursed me</button></div></>}
   {work.lastAction==='reimbursement'?<><h1>Send me the reimbursement records.</h1><p>Send the expense and reimbursement records so Betti can establish what this repaid. No business income or expense reversal has been assumed.</p><DocumentIntake guided={embedded} compact recordId={work.recordId}/></>:(!embedded||work.lastAction==='merchant_return'||work.candidates.length>0)&&<><PurchaseHeading className="text-lg font-semibold">{work.candidates.length===1?'Is it for this purchase?':'Which purchase was it for?'}</PurchaseHeading><p>This looks like a refund. I need you to confirm the original purchase.</p>{work.candidates.map(c=><label className="flex min-h-12 gap-3 rounded border p-3" key={c.recordId}><input type="radio" name="return-purchase" checked={selected===c.recordId} onChange={()=>{setSelected(c.recordId);setNone(false)}}/><span><MerchantIdentity compact merchant={c.merchant} date={c.date} amountCents={c.amountCents}/>{c.crossYear&&<span className="block">Different tax year — needs tax-treatment review</span>}</span></label>)}
   ...
   <button className="btn btn-secondary" onClick={()=>setNone(true)}>I don’t see it / something else</button>{none&&<><p>Send the original purchase or return record. I’ll keep this unresolved until the relationship is supported.</p><DocumentIntake guided={embedded} compact recordId={work.recordId}/></>}</>}
  </>}
  ...
  {work.kind==='loan'&&<><h1 className="text-2xl font-semibold">Send me the loan statement.</h1><p>I can see this was a loan payment, but I need the statement to separate principal and interest.</p><DocumentIntake guided={embedded} buttonLabel="Choose statement" recordId={work.recordId}/></>}
