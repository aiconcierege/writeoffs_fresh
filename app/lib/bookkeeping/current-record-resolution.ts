...
export async function loadCurrentRecordConvergences(input: {
  ...
  const [convergenceResult, compoundResult] = await Promise.all([
  ...
  if (convergenceResult.error || compoundResult.error) throw new Error(`Unable to resolve current bookkeeping identity: ${convergenceResult.error?.message ?? compoundResult.error?.message}`)
  ...
  return {
    ...
    isInactive(recordId: string) {
      return suppressedCompoundAnchorIds.has(recordId)
    },
