import type {WorkInputSnapshot} from './work-input-snapshot'
...
export async function loadCurrentCustomerWork(input:{supabase:SupabaseClient;scope?:'business'|'expenses';asOf?:string;recordId?:string;onSnapshot?:(snapshot:WorkInputSnapshot)=>void}) {
  ...
  const work=await loadBettiWork({db:input.supabase,businessId:business.id,scope,asOf:input.asOf,
    continuityRecordId:input.recordId,onSnapshot:input.onSnapshot,processingEnabled:process.env.DOCUMENT_EXPENSIVE_PROCESSING_ENABLED!=='false'})
