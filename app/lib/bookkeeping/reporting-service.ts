...
export async function getAuthenticatedCanonicalReport(input: {
  ...
  const [canonical, legacy, questions, categoryLabels, businessMilesMilli] = await Promise.all([
  ...
  return { ...report, businessMilesMilli, mileageDeductionCents: null as null,
