...
export async function loadSpecialWork(db:SupabaseClient,recordId:string):Promise<SpecialWork|null>{
 ...
 const kind=w.bookkeeping_nature==='refund'?'refund':w.bookkeeping_nature==='loan_principal_payment'?'loan':w.treatment==='unresolved'&&w.amount_cents<0&&/loan|payment|transfer/i.test(w.description??w.merchant??'')?'movement':null
