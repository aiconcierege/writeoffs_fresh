...
export async function processOperatingExpenseTreatment(input: {
  ...
  const evidenceFingerprint = fingerprint({ version: classification.version,
    ...
    taxFacts: classification.taxFacts })
  ...
  if ((classification.status === 'needs_facts'
    ...
    await new SupabaseBookkeepingRepository(admin).openReviewIssue({
      ...
      issueKey: `schedule-c-category:${snapshot.recordId}`,
      ...
      questionContext: { schemaVersion: 1, routingVersion: classification.version,
        reason: 'BUSINESS_PURPOSE_NEEDED', factType: classification.categoryKey === 'travel'
          ? 'business_travel_details' : 'ordinary_expense_purpose',
