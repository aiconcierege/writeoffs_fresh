...
export async function persistAccountUse(accountId: string, request: AccountUseRequest, onWork?:(work:GuidedWorkProjection|undefined)=>void): Promise<string> {
  const response = await fetch(`/api/bookkeeping/accounts/${accountId}/use`, {
    method: 'POST', headers: { 'Content-Type': 'application/json',...(onWork?{'x-betti-guided':'1'}:{}) },
    body: JSON.stringify(request), keepalive: true,
  })
  const body = await response.json().catch(() => null) as { ok?: boolean; eventId?: string;work?:GuidedWorkProjection } | null
