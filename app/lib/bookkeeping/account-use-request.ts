...
export async function persistAccountUse(accountId: string, request: AccountUseRequest, onWork?:(work:BettiWorkProjection|undefined)=>void): Promise<string> {
  ...
  const body = await response.json().catch(() => null) as { ok?: boolean; eventId?: string;work?:BettiWorkProjection } | null
