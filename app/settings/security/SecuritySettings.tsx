...
export function SecuritySettings({ enrollmentRequired, next }: { enrollmentRequired: boolean; next: string }) {
  ...
  return <main className="app-page"><div className="page-container page-container-narrow">
    <p className="eyebrow">{enrollmentRequired ? 'Set up WriteOffs' : 'Account'}</p><h1 className="page-title">{enrollmentRequired ? 'Protect your account' : 'Security'}</h1><p className="page-description">{enrollmentRequired ? 'WriteOffs keeps financial information in your account, so we require an authenticator app when you sign in.' : 'Protect your financial records with an authenticator app.'}</p>
    {enrollmentRequired && !verifiedFactorId && <p className="mt-5 text-sm text-[#59665f]">It only takes a minute.</p>}
    <section className={enrollmentRequired?"mt-7":"section-rule mt-9"} aria-labelledby="two-factor-heading">{enrollmentRequired?<h2 id="two-factor-heading" className="sr-only">Set up an authenticator app</h2>:<div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between"><div><h2 id="two-factor-heading" className="section-heading">Two-factor authentication</h2>
      ...
      {enrollment && <form onSubmit={verifyEnrollment} className="mt-6 py-5 sm:py-7"><h3 className="font-semibold text-[#17211d]">Scan the QR code</h3><p className="mt-2 text-sm leading-6 text-[#59665f]">Scan this with your authenticator app, then enter its 6-digit code.</p>
        ...
        <details className="mt-4 text-sm"><summary className="cursor-pointer font-semibold text-[#243186]">On the same phone, or can’t scan the code?</summary><p className="mt-2 text-sm">Copy this setup key into your authenticator app, then return here to enter its code.</p><p className="mt-2 break-all rounded-lg bg-[#f0f5f1] p-3 font-mono text-xs" aria-label="Authenticator setup key">{enrollment.secret}</p></details>
    ...
    <section className="section-rule mt-10"><h2 className="text-sm font-semibold">Keep access to your account</h2><p className="mt-3 text-sm leading-6 text-[#59665f]">Keep access to your authenticator app. If you lose access, you’ll need to contact WriteOffs support to recover your account.</p></section>
  </div></main>
}
