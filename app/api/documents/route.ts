...
async function registerDocument(request:Request){
 const db=await createServerSupabase(),{data:{user}}=await db.auth.getUser();if(!user)return NextResponse.json({error:'unauthorized'},{status:401})
 ...
 const object=await timed('document_storage_verification',()=>db.storage.from('receipts').info(`receipts/${user.id}/${b.fingerprint}`))
...
export const POST=timedRoute(registerDocument)
