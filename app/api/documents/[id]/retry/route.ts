...
export async function POST(_request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const job=await admin.from('receipt_processing_jobs').select('id,state,terminal_reason').eq('document_id',id).eq('business_id',businessId).order('created_at',{ascending:false}).limit(1).maybeSingle()
    if(job.error||!job.data)throw new Error('JOB_UNAVAILABLE')
    if(['dead_letter','unreadable'].includes(job.data.state)||(job.data.state==='needs_attention'&&job.data.terminal_reason?.startsWith('LOAN_'))) {
