...
export function ReportsSummary() {
  ...
  return <main className="page-container max-w-5xl space-y-11">
    ...
    <dl className="grid border-t border-[#dce3de] sm:grid-cols-2 sm:gap-x-10">
      {metrics.map(([label, amount], index) => <div key={label} className={`border-b border-[#dce3de] py-5 ${index === 2 ? 'sm:col-span-2 sm:flex sm:items-baseline sm:justify-between' : ''}`}>
        <dt className="text-sm text-[#59665f]">{label}</dt><dd className={`money-display mt-2 ${index === 2 ? 'text-4xl font-semibold sm:mt-0' : 'text-3xl font-semibold'}`}>{usd.format(amount / 100)}</dd></div>)}
    </dl>
