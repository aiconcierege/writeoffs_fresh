...
export default async function TaxTimePage({ searchParams }: { searchParams: Promise<{ year?: string }> }) {
  ...
  return <main className="mx-auto max-w-5xl px-3 py-8 sm:px-6 sm:py-10">
    <header className="max-w-3xl"><p className="text-xs font-semibold tracking-[0.16em] text-slate-500">ANNUAL RECORDS</p>
      <div className="mt-2 flex flex-wrap items-end justify-between gap-4"><div><h1 className="text-3xl font-semibold tracking-tight text-slate-950">{year} tax-time readiness</h1>
        <p className="mt-3 text-lg font-medium text-slate-800">{labels[readiness.status]}</p></div>
    ...
    <section className="mt-9 grid gap-3 sm:grid-cols-2 lg:grid-cols-3" aria-label="Readiness details">
      {readiness.dimensions.map(dimension => <article key={dimension.key} className="rounded-xl border border-slate-200 bg-white p-5"><div className="flex items-center justify-between gap-3"><h2 className="font-semibold text-slate-950">{dimension.label}</h2><span className="text-xs font-semibold uppercase tracking-wide text-slate-500">{labels[dimension.status]}</span></div><p className="mt-2 text-sm leading-6 text-slate-600">{dimension.summary}</p></article>)}
    </section>
    ...
    {processingIssues.length > 0 && <section className="mt-9 rounded-xl bg-slate-50 p-5"><h2 className="font-semibold text-slate-950">WriteOffs is still working</h2>{processingIssues.map((issue,index)=><p key={index} className="mt-2 text-sm text-slate-600">{issue.detail}</p>)}</section>}
    <section className="mt-10 border-t border-slate-200 pt-8"><h2 className="text-xl font-semibold text-slate-950">Download tax records</h2><p className="mt-2 text-sm text-slate-600">Download current canonical records and factual summaries for your tax preparer.</p>
      <div className="mt-4 flex flex-wrap gap-3 text-sm font-semibold text-[#243186]"><a href={`/api/export/csv?year=${year}`}>Transaction CSV</a><a href={`/api/mileage/export?year=${year}`}>Mileage CSV</a><a href={`/api/contractors/export?year=${year}`}>Contractor CSV</a><a href={`/api/reports/annual-package?year=${year}&file=documentation`}>Documentation summary</a><a href={`/api/reports/annual-package?year=${year}&file=unresolved-items`}>Unresolved items</a><Link href={`/reports/schedule-c?year=${year}`}>Schedule C categories</Link></div>
    </section>
