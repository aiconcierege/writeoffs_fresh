  ...
  return <main className="app-page"><article className="page-container page-container-narrow">
    ...
    <section className="grid gap-6 border-b border-slate-200 py-6 sm:grid-cols-2 sm:gap-8 sm:py-8">
      <div><h2 className="text-lg font-semibold text-slate-950">{specialNatureLabel(transaction.bookkeepingNature,transaction.treatment)??'How Betti handled this'}</h2>
        ...
        <p className="mt-2 text-sm leading-6 text-slate-600">{transaction.compoundParts?'The statement supports this split. Principal is outside business expenses; supported business interest is included separately. Tax treatment is assessed separately.':progress.message}</p>
        {transaction.compoundParts&&<dl className="mt-3 space-y-3 text-sm">{transaction.compoundParts.map(part=><div key={part.recordId}><dt>{part.label}</dt><dd>{money.format(part.amountCents/100)} · <Link className="underline" href={withReturnTo(`/transactions/${part.recordId}`,detailContext)}>View part and history</Link></dd></div>)}</dl>}
        ...
        {transaction.bookkeepingNature==='refund'&&transaction.treatment!=='unresolved'&&<Link className="btn btn-secondary" href={withReturnTo(`/check-in?record=${transaction.recordId}&review=1`,detailContext)}>Review return relationship</Link>}
        {(transaction.bookkeepingNature==='credit_card_payment'||(transaction.bookkeepingNature==='transfer'&&transaction.treatment==='personal'))&&<Link className="btn btn-secondary" href={withReturnTo(`/check-in?record=${transaction.recordId}&review=1`,detailContext)}>Review payment type</Link>}
        ...
        {transaction.treatment==='mixed_use'&&<dl className="mt-3 text-sm"><dt>Business portion</dt><dd>{money.format((transaction.businessAmountCents??0)/100)}</dd><dt>Personal portion</dt><dd>{money.format((transaction.personalAmountCents??0)/100)}</dd></dl>}
        {transaction.treatment==='personal'&&<p className="mt-3 text-sm">Money taken or used personally: {money.format((transaction.personalAmountCents??Math.abs(transaction.amountCents))/100)}. Not a business expense.</p>}
        ...
        {transaction.sourceModel==='canonical'&&transaction.currentDecisionId&&transaction.bookkeepingNature==='expense'&&transaction.treatment==='personal'
          ...
          :transaction.sourceModel==='canonical'&&transaction.currentDecisionId&&transaction.bookkeepingNature==='expense'&&transaction.treatment==='excluded'&&transaction.decisionProvenance==='user'
