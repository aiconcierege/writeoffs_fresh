...
export function TransactionReview({rows,view,historical,returnTo='/transactions'}:{rows:Row[];view:WorkView;historical:boolean;returnTo?:string}) {
  ...
  return <>
    ...
    {rows.length>0&&<div className="review-selection-bar"><h2>Activity</h2><button className="btn btn-secondary" aria-pressed={selectionMode} disabled={busy} onClick={()=>{setSelectionMode(!selectionMode);setSelected(new Set());setConfirm(null)}}>{selectionMode?'Done selecting':'Select'}</button></div>}
    {selectionMode&&rows.length>0&&<div className="review-select-heading"><label className="review-checkbox-label"><input type="checkbox" checked={all} ref={element=>{if(element)element.indeterminate=chosen.length>0&&!all}} disabled={busy} onChange={()=>{setConfirm(null);setSelected(all?new Set():new Set(rows.map(row=>row.id)))}}/>Select all on this page</label><span role="status" aria-live="polite">{chosen.length>0?`${chosen.length} selected`:''}</span></div>}
    ...
    <div className="record-list transaction-records">
      {rows.map(row=>{const secondary=row.has_receipt?attachedReceiptLabel(row):row.receiptLost?'Receipt unavailable':row.sourceLabel??(purchaseReceiptEligible(row)?'No receipt attached':null);return <div key={row.id} className={`review-transaction-row ${selected.has(row.id)?'is-selected':''}`}>
        {selectionMode&&<label className="review-row-select"><input type="checkbox" aria-label={`Select ${row.vendor}, ${row.date}, ${money(row)}`} checked={selected.has(row.id)} disabled={busy} onChange={()=>toggle(row.id)}/></label>}
        <Link href={row.sourceKind==='receipt_evidence'?'/receipts':withReturnTo(`/transactions/${row.id}`,returnTo)} className="review-row-open" aria-label={`Open ${row.vendor}, ${row.date}, ${money(row)}`}>
          <div className="min-w-0"><p className="transaction-merchant">{row.vendor}</p><p className="transaction-row-meta">{new Intl.DateTimeFormat('en-US',{month:'short',day:'numeric',year:'numeric',timeZone:'UTC'}).format(new Date(`${row.date}T00:00:00Z`))} <span className={row.treatmentLabel==='Business'?'transaction-business-status':'transaction-status'}>{row.treatmentLabel}</span></p>{secondary&&<p className="mt-1 text-xs text-[#59665f]">{secondary}</p>}</div>
