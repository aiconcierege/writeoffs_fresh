...
export function TransactionReview({rows,view,historical}:{rows:Row[];view:WorkView;historical:boolean}) {
  ...
  const canReceipt=canRemove&&chosen.every(row=>!row.has_receipt&&!row.receiptLost&&row.amountCents<0&&['expense','unresolved'].includes(row.bookkeepingNature??''))
  ...
  async function apply(action:NonNullable<typeof confirm>) {
    ...
    const scope=historical?'historical':view==='receipt-only'?'all':view
  ...
  return <>
    ...
    {rows.length>0&&<div className="review-select-heading"><label className="review-checkbox-label"><input type="checkbox" checked={all} ref={element=>{if(element)element.indeterminate=chosen.length>0&&!all}} disabled={busy} onChange={()=>{setConfirm(null);setSelected(all?new Set():new Set(rows.map(row=>row.id)))}}/>Select all on this page</label><span role="status" aria-live="polite">{chosen.length} selected</span></div>}
    {chosen.length>0&&<section className="review-bulk-actions" aria-label="Selected purchase actions"><p className="font-semibold" aria-live="polite">{chosen.length} selected on this page</p>
      {confirm?<><p>{confirm==='receipt_unavailable'?`You don’t have receipts for these ${chosen.length} purchases. I’ll record that fact and keep the other evidence we have.`:confirm==='reviewed'?`You’ve checked these ${chosen.length} purchases. This records your review without changing their bookkeeping or evidence.`:`Remove these ${chosen.length} purchases from your business? Their bank records and history will be kept.`}</p><div className="flex flex-wrap gap-2"><button className="btn btn-primary" disabled={busy} onClick={()=>void apply(confirm)}>{busy?'Saving…':confirm==='receipt_unavailable'?'I don’t have these receipts':confirm==='reviewed'?'I’ve reviewed these purchases':'Remove from business'}</button><button className="btn btn-secondary" disabled={busy} onClick={()=>setConfirm(null)}>Cancel</button></div></>:<div className="flex flex-wrap gap-2">
        {canReceipt&&<button className="btn btn-primary" onClick={()=>setConfirm('receipt_unavailable')}>Continue with {chosen.length} {chosen.length===1?'purchase':'purchases'}</button>}
        <button disabled={!canRemove} className={`btn ${canReceipt?'btn-secondary':'btn-primary'}`} onClick={()=>setConfirm('remove_business')}>Remove from business</button>
        {historical&&<button disabled={!canRemove} className="btn btn-secondary" onClick={()=>setConfirm('reviewed')}>Mark selected as reviewed</button>}
        <button className="min-h-11 px-2 text-sm underline" onClick={()=>setSelected(new Set())}>Clear selection</button>
      </div>}
