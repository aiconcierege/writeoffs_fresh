...
export function MileageClient({ initialVehicles, initialEntries,initialVehicleIdentities=[],initialVehicleMethods=[],initialVehicleUseFacts=[] }: { initialVehicles:Vehicle[];initialEntries:Entry[];initialVehicleIdentities?:VehicleIdentity[];initialVehicleMethods?:VehicleMethod[];initialVehicleUseFacts?:VehicleUse[] }) {
  ...
  const singleIdentity=active.length===1?identities.find(identity=>identity.vehicle_id===active[0].id):null
  const singleMethod=active.length===1?methods.find(method=>method.vehicle_id===active[0].id&&method.tax_year===new Date().getFullYear()):null
  ...
  return <AuthenticatedPage className={`mileage-page signature-page ${firstUse?'mileage-first-use':''}`} eyebrow="Business mileage" title={firstUse?'Let’s set up your vehicle.':'Mileage'} description={firstUse?'Tell me which vehicle you use for business. You’ll only need to do this once.':'Every business trip, kept together.'} actions={firstUse?<BettiPresence state="question" className="vehicle-setup-betti" sizes="(max-width: 639px) 120px, 360px"/>:entries.length>0?<div className="mileage-log-overview"><div><span className="workspace-kicker">In this log</span><strong>{formatMiles(recordedMiles)} <small>business miles</small></strong></div><p>{entries.length} {entries.length===1?'trip recorded':'trips recorded'}</p></div>:undefined}>
    <div className={`mileage-workspace ${!firstUse?'workspace-surface':''}`}>
     ...
     {!firstUse&&<aside className="mileage-vehicle-context" aria-label="Your vehicles">
      ...
      {singleIdentity&&singleIdentity.ownership!=='unknown'&&singleMethod&&['actual_expenses','standard_mileage'].includes(singleMethod.method)&&<dl className="vehicle-summary"><div><dt>Ownership</dt><dd>{singleIdentity.ownership==='leased'?'Leased':'Owned'}</dd></div><div><dt>Tracking in {singleMethod.tax_year}</dt><dd>{singleMethod.method==='actual_expenses'?'Vehicle costs':'Business miles'}</dd></div></dl>}
