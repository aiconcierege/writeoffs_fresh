...
export function MileageClient({ initialVehicles, initialEntries,initialVehicleIdentities=[],initialVehicleMethods=[],initialVehicleUseFacts=[] }: { initialVehicles:Vehicle[];initialEntries:Entry[];initialVehicleIdentities?:VehicleIdentity[];initialVehicleMethods?:VehicleMethod[];initialVehicleUseFacts?:VehicleUse[] }) {
  ...
  return <AuthenticatedPage className={`mileage-page premium-utility ${firstUse?'mileage-first-use':''}`} eyebrow="Business mileage" title={firstUse?'Let’s set up your vehicle.':'Mileage'} description={firstUse?'Tell me which vehicle you use for business. You’ll only need to do this once.':'Your business driving, all in one place.'} actions={firstUse?<BettiPresence state="question" className="vehicle-setup-betti" sizes="(max-width: 639px) 120px, 240px"/>:undefined}>
    ...
    <div className="mt-4">{active.length>0&&<button onClick={()=>setAddingVehicle((v)=>!v)} className="min-h-11 text-sm font-semibold text-[#243186]">{addingVehicle?'Back to trips':'Add vehicle'}</button>}</div>
    </div>
    ...
    {!firstUse&&<section className="mt-9 border-t border-slate-200 pt-6" aria-labelledby="trips-heading"><div className="flex flex-wrap items-center justify-between gap-3"><h2 id="trips-heading" className="text-lg font-semibold">Recorded trips</h2><a href={`/api/mileage/export?year=${new Date().getFullYear()}`} className="inline-flex min-h-11 items-center text-sm font-semibold text-[#243186]">Download mileage</a></div>
