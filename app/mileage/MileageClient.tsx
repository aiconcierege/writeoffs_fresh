...
export function MileageClient({ initialVehicles, initialEntries }: { initialVehicles:Vehicle[];initialEntries:Entry[] }) {
  ...
  return <main className="mx-auto max-w-3xl px-2 py-8 sm:px-6 sm:py-12">
    <header><p className="text-xs font-semibold tracking-[0.16em] text-slate-500">BUSINESS DRIVING</p>
      <h1 className="mt-2 text-3xl font-semibold tracking-tight text-slate-950">Mileage</h1>
    ...
    <section className="mt-9 border-t border-slate-200 pt-6" aria-labelledby="trips-heading"><h2 id="trips-heading" className="text-lg font-semibold">Recorded trips</h2>
      {entries.length===0?<p className="mt-4 text-sm text-slate-600">No mileage recorded yet.</p>:<div className="mt-4 grid gap-3">{entries.map((entry)=><article key={entry.id} className="rounded-xl border border-slate-200 bg-white p-4">
...
function MileageForm({vehicles,entry,onDone}:{vehicles:Vehicle[];entry:Entry|null;onDone:()=>Promise<void>}){
  ...
  return <form key={entry?.id??'new'} className="mt-7 rounded-xl border border-slate-200 bg-white p-4 sm:p-6" onSubmit={async(e)=>{e.preventDefault();setBusy(true);setError(null);const f=new FormData(e.currentTarget)
    ...
    </div>{error&&<p role="alert" className="mt-3 text-sm text-red-700">{error}</p>}<button disabled={busy} className="mt-5 min-h-12 w-full rounded-md bg-[#243186] px-5 text-sm font-semibold text-white disabled:opacity-50 sm:w-auto">{busy?'Saving…':entry?'Save correction':'Save mileage'}</button>
  </form>}

function VehicleForm({onDone}:{onDone:()=>Promise<void>}){const [error,setError]=useState<string|null>(null);return <form className="mt-7 rounded-xl border border-slate-200 bg-white p-4 sm:p-6" onSubmit={async(e)=>{e.preventDefault();const f=new FormData(e.currentTarget);const response=await fetch('/api/mileage/vehicles',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({displayName:f.get('displayName'),vehicleYear:f.get('vehicleYear'),make:f.get('make'),model:f.get('model'),isMixedUse:f.get('isMixedUse')==='yes'})});const data=await response.json().catch(()=>({}));if(!response.ok){setError(data.error??'Vehicle could not be added.');return}await onDone()}}>
