...
`/check-in` renders the authoritative Betti work projection, including guided actions, not a second question queue. Account-use remains one canonical fact. Historical work follows the statements, receipts, personal-exception and non-expense-exception stages in [Catch-up Workflow V2](CATCH_UP_WORKFLOW_V2.md) before its remaining substantive questions. Ongoing work retains evidence-driven specific questions and optional exception review, without a universal partly-personal stage. Mixed-account use is never presumed business. Already customer-established use and valid existing mixed/multiple-category allocations are not reclassified by a generic sweep.

Historical exception-review pages contain at most twenty visible items; ongoing grouped reviews retain their existing bounded size. Explicit answers are append-only, versioned snapshots of record, decision, evidence and account-use identity. New/unseen activity never inherits a prior review. Personal/mixed answers invoke existing canonical correction commands. No selection in a business-only exception sweep records only that scoped review, not a fabricated new business-use or tax decision.

Receipt upload uses the unified intake. Relevant processing must settle before receipt-availability confirmation. Historical receipt collection requires explicit done/no-receipts/later acknowledgment after relevant processing; upload alone does not finish collection. These journey events do not invent missing-receipt facts or erase working expense treatment. Existing ongoing receipt-availability assertions remain scoped to their displayed eligible group. Deferral is separate from completion. Reassessment and matching remain canonical workers; read/render operations do not mutate them.

Engaged customers continue naturally while legitimate actions remain. The current authoritative substantive set is shown as “questions left,” and the counter disappears at zero. It may change with evidence or answers; it is not a fixed promise. There is no automatic five-action stop. Receipt Later defers the displayed opportunity without asserting unavailability; “That’s all the receipts I have” is a separate versioned assertion. Projection priority and dependencies determine the next useful action. Completion language distinguishes the end of a visit, actual processing, deferred work and supported source coverage; it never invents whole-business current-through or Catch-up review authority.
