...
try {
 ...
 stage='early candidate';await page.goto(origin+`/transactions/${fixture('ADOBE CREATIVE CLOUD').transactionId}`)
 ...
 stage='statement account use';await page.goto(origin+'/check-in')
 ...
 drain()
 stage='promoted category';await page.goto(origin+`/transactions/${fixture('ADOBE CREATIVE CLOUD').transactionId}`)
 ...
 stage='customer answer';await page.goto(origin+`/check-in?record=${fixture('FUN').recordId}`)
 ...
 stage='money in';await answer('ZELLE FROM ROBERT HALL','Payment from a customer','business_income')
 stage='refund';await answer('REFUND - OFFICE DEPOT','Refund or reimbursement','refund')
 stage='card payment';await answer('ACH PAYMENT - BUSINESS CREDIT CARD 3333','A credit card payment','credit_card_payment')
 stage='reconcile totals';r=await report();checkMath(r);assert.equal(r.businessIncomeCents,42500)
 stage='receipt unavailable';await page.goto(origin+'/transactions?view=receipts')
 ...
 stage='tenant isolation';const other=await session(fixtures[1],browser)
