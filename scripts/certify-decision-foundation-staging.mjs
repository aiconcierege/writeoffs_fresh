...
try {
 ...
 await page.goto(origin+'/reports');await page.getByText('Still being categorized',{exact:true}).waitFor();await page.screenshot({path:`${dir}/proof/uncategorized-before.png`,fullPage:true})
 ...
 stage='responsive';console.log('Certification: responsive');for(const width of [390,430,768,1280]){await page.setViewportSize({width,height:900});for(const[path,name]of[['/home','home'],['/reports','reports'],['/reports/tax-time','tax-time'],[`/transactions/${fixture('ADOBE CREATIVE CLOUD').transactionId}`,'software'],[`/check-in?record=${fixture('EQUIPMENT LOAN PAYMENT').recordId}`,'question'],[`/transactions/${fixture('EQUIPMENT LOAN PAYMENT').transactionId}`,'loan']]){await page.goto(origin+path);if(name==='reports')await page.getByRole('heading',{name:'Where the money went',exact:true}).waitFor();else await page.locator('main').last().waitFor();await page.waitForTimeout(250);assert(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth));await page.screenshot({path:`${dir}/proof/${name}-${width}.png`,fullPage:true})}}
