...
async function main(){
 ...
 try{
  ...
  assert.equal(flow.length,15);assert.equal(flow.filter(a=>a.type==='personal_exception_sweep').length,1)
  ...
  if(mode==='--hosted'){
   ...
   await page.getByRole('heading',{name:'Have receipts? Send them first.',exact:true}).waitFor()
   assert.equal(await page.locator('[data-guided-action]').getAttribute('data-guided-action'),work.nextAction?.type)
   assert.equal(await page.locator('[data-customer-action-count]').getAttribute('data-customer-action-count'),'15')
