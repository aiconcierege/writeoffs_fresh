...
export function seedSyntheticTenant(t){
 ...
 insert into public.plaid_items(business_id,plaid_item_id,access_token_ciphertext,environment,sync_cursor,sync_requested_at,sync_lease_id,sync_lease_expires_at,new_accounts_available,update_reason,update_state_version,update_notice_at) values('${t.business}','synthetic-${t.business}','synthetic-not-a-provider-token','sandbox','synthetic-cursor',now(),gen_random_uuid(),now()+interval '1 hour',true,'PENDING_DISCONNECT',1,now());
