...
async function main(){
 ...
 for(const tenant of ['a','b']){
  const nonce=randomUUID(),email=`foundation-${tenant}-${nonce}@staging.writeoffs.invalid`,password=`Proof-${nonce}!`
  const created=await admin.auth.admin.createUser({email,password,email_confirm:true,user_metadata:{synthetic_foundation:true}});assert(!created.error&&created.data.user)
  ...
  assert(!(await admin.rpc('create_business_membership_grant',{p_business_id:businessId,p_plan:'business',p_starts_at:'2025-01-01T00:00:00Z',p_ends_at:null,p_request_key:`foundation:${nonce}`,p_reason:'Isolated decision foundation certification',p_provenance:'admin',p_actor_user_id:null})).error)
  ...
  const account=await admin.from('financial_accounts').insert({business_id:businessId,institution_name:'Synthetic receipt test bank',provider:'statement',provider_account_id:`foundation:${nonce}`,display_name:'Statement checking',account_type:'checking',currency:'USD'}).select('id').single();assert(account.data)

  const trusted=new SupabaseBookkeepingRepository(admin),writer=new SupabaseBookkeepingRepository(customer),records=[]
  const cases:Array<[string,number,string]>=tenant==='a'?[['ADOBE CREATIVE CLOUD',-2299,'2026-05-03'],['OFFICE DEPOT',-6419,'2026-05-12'],['FUN',-8940,'2026-05-07'],['VERIZON',-14628,'2026-05-08'],['KNOWN RESTAURANT MEAL',-433,'2026-05-09'],['REFUND - OFFICE DEPOT',3210,'2026-05-28'],['ZELLE FROM ROBERT HALL',42500,'2026-05-07'],['ACH PAYMENT - BUSINESS CREDIT CARD 3333',-128437,'2026-05-10'],['EQUIPMENT LOAN PAYMENT',-45000,'2026-05-15']]:[]
  for(const [merchant,amount,day] of cases){const amountCents=Number(amount),occurredOn=String(day),fingerprint=`foundation:${randomUUID()}`
 ...
 await writeFile('/private/tmp/writeoffs-foundation/fixtures.json',JSON.stringify(fixtures),{mode:0o600})
