...
async function main(){
 ...
 for(const tenant of ['a','b']){
  const nonce=randomUUID(),email=`receipt-repair-${tenant}-${nonce}@staging.writeoffs.invalid`,password=`Proof-${nonce}!`
  const created=await admin.auth.admin.createUser({email,password,email_confirm:true,user_metadata:{synthetic_receipt_repair:true}});assert(!created.error&&created.data.user)
  ...
  const cases=tenant==='a'?[["McDonald's",-1200,'2026-09-08'],['STARBUCKS',-433,'2026-09-08'],['UBER',-540,'2026-09-11'],['Ambiguous Shop',-2500,'2026-09-08'],['Ambiguous Shop',-2500,'2026-09-09'],['Date Tolerance Shop',-1865,'2026-09-10']]:[["McDonald's",-1200,'2026-09-08']]
 ...
 await writeFile('/private/tmp/writeoffs-receipt-repair/fixtures.json',JSON.stringify(fixtures),{mode:0o600})
 console.log('Created two isolated synthetic tenants and seven bank-backed business purchases. No receipts or extraction results seeded.')
}
