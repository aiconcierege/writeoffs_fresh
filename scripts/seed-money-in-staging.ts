...
async function main(){
 ...
 for(const tenant of ['a','b']){
  const nonce=randomUUID(),email=`money-in-${tenant}-${nonce}@staging.writeoffs.invalid`,password=`Proof-${nonce}!`
  const created=await admin.auth.admin.createUser({email,password,email_confirm:true,user_metadata:{synthetic_money_in:true}});assert(!created.error&&created.data.user)
  ...
  const cases:Array<[string,number,string]>=tenant==='a'?[['ZELLE FROM ROBERT HALL',42500,'2026-05-07'],['STRIPE PAYOUT',73544,'2026-05-11'],['TRANSFER FROM SAVINGS',50000,'2026-05-09'],['LOAN PROCEEDS',200000,'2026-05-12'],['OWNER FUNDS',10000,'2026-05-13'],['REFUND',1500,'2026-05-14'],['OFFICE DEPOT',-6419,'2026-05-15'],['PAPER STORE',-1800,'2026-05-16']]:[]
 ...
 await writeFile('/private/tmp/writeoffs-money-in/fixtures.json',JSON.stringify(fixtures),{mode:0o600})
 console.log('Created two isolated money-in tenants with unresolved deposits and explicit synthetic purchase facts.')
}
