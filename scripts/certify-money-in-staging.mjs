...
try {
 ...
 stage='totals';for(const path of ['/home','/reports?year=2026']) {await page.goto(origin+path);assert.equal(await page.locator('dt').filter({hasText:/^Business income$/}).locator('..').locator('dd').innerText(),'$425.00');await page.screenshot({path:`${dir}/proof/${path.startsWith('/home')?'home':'reports'}-income.png`,fullPage:true})}
 stage='bulk';await page.goto(origin+'/transactions?view=receipts');await page.getByRole('checkbox',{name:'Select all on this page'}).check()
