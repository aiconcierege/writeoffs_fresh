...
a:=public.open_bookkeeping_review_issue_v2(r.business_id,r.id,r.decision_id,'TRANSACTION_TYPE_UNCLEAR','refresh-proof',repeat('a',64),'{"schemaVersion":1,"reason":"TRANSACTION_TYPE_UNCLEAR","factType":"money_in_source"}'::jsonb);
...
b:=public.open_bookkeeping_review_issue_v2(r.business_id,r.id,r.decision_id,'TRANSACTION_TYPE_UNCLEAR','refresh-proof',repeat('a',64),'{"schemaVersion":1,"reason":"TRANSACTION_TYPE_UNCLEAR","factType":"money_in_source"}'::jsonb);
if a=b then raise exception 'changed evidence did not refresh';end if;
c:=public.open_bookkeeping_review_issue_v2(r.business_id,r.id,r.decision_id,'TRANSACTION_TYPE_UNCLEAR','refresh-proof',repeat('a',64),'{"schemaVersion":1,"reason":"TRANSACTION_TYPE_UNCLEAR","factType":"money_in_source"}'::jsonb);
...
perform set_config('request.jwt.claims','{"role":"service_role"}',true);
if public.open_bookkeeping_review_issue_v2(r.business_id,r.id,r.decision_id,'TRANSACTION_TYPE_UNCLEAR','refresh-proof',repeat('b',64),'{"schemaVersion":1,"reason":"TRANSACTION_TYPE_UNCLEAR","factType":"money_in_source"}'::jsonb)<>c then raise exception 'deferred issue reopened';end if;
b:=public.resolve_bookkeeping_review_issue(r.business_id,issue,c);
if public.open_bookkeeping_review_issue_v2(r.business_id,r.id,r.decision_id,'TRANSACTION_TYPE_UNCLEAR','refresh-proof',repeat('c',64),'{"schemaVersion":1,"reason":"TRANSACTION_TYPE_UNCLEAR","factType":"money_in_source"}'::jsonb)<>b then raise exception 'resolved issue reopened';end if;
