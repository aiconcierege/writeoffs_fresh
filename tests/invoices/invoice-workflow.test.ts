...
describe('invoice workflow contracts', () => {
  ...
  it('provides a mobile-first optional invoice flow and safe share artifact', () => {
    ...
    expect(list).toContain('I’ll add the income to your books when you get paid.')
    expect(list).toContain('WorkspaceGroup')
    expect(list).toContain('name="amount"')
    expect(list).toContain('inputMode="decimal"')
