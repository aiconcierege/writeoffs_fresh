...
function client(business = complete(), count = 1) {
  ...
  return { api: { auth: { getUser: vi.fn(async () => ({ data: { user: { id: 'u1' } }, error: null })) }, from: vi.fn(() => businesses) }, updates, mutation, businesses, ownerEq }
}
...
describe('POST /api/onboarding/complete', () => {
  ...
  it('sets v3 completion once and returns Home', async () => {
    const c = client(); createServerSupabase.mockResolvedValue(c.api)
    const response = await POST(); expect(response.status).toBe(200)
    expect(c.updates[0]).toMatchObject({ onboarding_state: 'completed', onboarding_version: 3, onboarding_completed_at: expect.any(String) })
    expect(c.mutation.eq).toHaveBeenCalledWith('id', 'b1'); expect(c.mutation.eq).toHaveBeenCalledWith('owner_user_id', 'u1')
  ...
  it('is idempotent after completion', async () => {
    ...
    createServerSupabase.mockResolvedValue(c.api)
    expect((await POST()).status).toBe(200); expect(c.businesses.update).not.toHaveBeenCalled()
  })
