...
describe('Home canonical visual derivations', () => {
  ...
  it('replaces historical charting with current operating status', () => {
    ...
    expect(home).not.toContain('<HomeOperatingStatus')
  ...
  it('keeps canonical Betti in a responsive conversational hero', () => {
    const styles = readFileSync('app/home/command-center.css', 'utf8')
    const hero = readFileSync('app/home/HomeBettiHero.tsx', 'utf8')
    expect(hero).toContain('<BettiPresence state={artworkState[projection.state]}')
    expect(styles).toContain('height:22rem')
    expect(styles).toContain('height:9.5rem')
    expect(styles).toContain('@media(max-width:639px)')
    expect(readFileSync('app/components/experience/experience.css', 'utf8')).toContain('animation: none')
  })
})
