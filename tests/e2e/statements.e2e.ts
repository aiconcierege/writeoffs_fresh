...
test('statement survives departure, imports once, and remains usable on mobile',async({page})=>{
  ...
  try{
    ...
    // Keep this browser test deterministic even when a developer's local queue
    // contains older retryable fixtures from prior integration runs.
    const prioritized=await admin.from('receipt_processing_jobs').update({available_at:'1970-01-01T00:00:00.000Z'})
      .eq('business_id',business.id).eq('job_type','statement_inspection')
    expect(prioritized.error).toBeNull()
    await expect.poll(async()=>{const drain=await page.request.post('/api/internal/processing/drain',{headers:{authorization:`Bearer ${workerSecret}`}});expect(drain.ok()).toBe(true);const job=(await admin.from('receipt_processing_jobs').select('state,last_error_code,attempt_count').eq('business_id',business.id).eq('job_type','statement_inspection').single()).data;if(job&&['dead_letter','unreadable','needs_attention','retryable'].includes(job.state))throw new Error(`Statement worker stopped: ${job.state}/${job.last_error_code??'unknown'}`);return (await admin.from('financial_transactions').select('id').eq('business_id',business.id).eq('import_method','statement')).data?.length??0},{timeout:20_000,intervals:[0,250,500,1_000]}).toBe(1)
