...
suite('contractor awareness against local PostgreSQL',()=>{it('tracks exact current payments, corrections, removal, W-9 awareness, idempotency, and isolation',async()=>{
 ...
 summary=(await listContractorSummaries({supabase:owner.customer,businessId:owner.businessId,taxYear:2026}))[0];expect(summary.awareness).toBe('potential_1099_attention')
 ...
 summary=(await listContractorSummaries({supabase:owner.customer,businessId:owner.businessId,taxYear:2026}))[0];expect(summary.totalPaidCents).toBe(200000);expect(summary.paymentCount).toBe(2)
