...
suite('contractor awareness against local PostgreSQL',()=>{it('tracks exact current payments, corrections, removal, W-9 awareness, idempotency, and isolation',async()=>{
 ...
 const beforeDefer=await getCurrentAskableQuestionQueue({supabase:owner.customer,scope:'business'})
 expect(beforeDefer.questions.some(question=>question.id===unknown.id&&question.source==='contractor')).toBe(true)
 const deferredUntil=new Date(Date.now()+7*86_400_000).toISOString()
 const deferKey=`contractor-defer:${crypto.randomUUID()}`
 expect((await owner.customer.rpc('defer_contractor_question',{p_question_source:'payment_method',p_question_id:unknown.id,
  p_expected_source_version_id:unknown.id,p_deferred_until:deferredUntil,p_request_key:deferKey})).error).toBeNull()
 const afterDefer=await getCurrentAskableQuestionQueue({supabase:owner.customer,scope:'business'})
 expect(afterDefer.questions.some(question=>question.id===unknown.id)).toBe(false)
 expect((await owner.customer.from('contractor_question_deferral_events').select('id').eq('request_key',deferKey)).data).toHaveLength(1)
 const afterExpiry=await getCurrentAskableQuestionQueue({supabase:owner.customer,scope:'business',asOf:'2030-01-01T00:00:00.000Z'})
 expect(afterExpiry.questions.some(question=>question.id===unknown.id&&question.version===unknown.id)).toBe(true)
 ...
 expect((await other.customer.rpc('defer_contractor_question',{p_question_source:'payment_method',p_question_id:unknown.id,
  p_expected_source_version_id:unknown.id,p_deferred_until:new Date(Date.now()+86_400_000).toISOString(),
  p_request_key:crypto.randomUUID()})).error).not.toBeNull()
