...
describe('transaction factual correction contract', () => {
  it('accepts only Business, Personal, or Both with a positive personal amount', () => {
    ...
    expect(validateTransactionUseCorrection({ schemaVersion: 1, use: 'restore_previous' }))
      .toEqual({ schemaVersion: 1, use: 'restore_previous' })
    expect(validateTransactionUseCorrection({ schemaVersion: 1, use: 'restore_exclusion' }))
      .toEqual({ schemaVersion: 1, use: 'restore_exclusion' })
