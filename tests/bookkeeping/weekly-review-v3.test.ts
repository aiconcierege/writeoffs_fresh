...
describe('Betti-led Weekly Review v3', () => {
    const candidate = (overrides: Partial<WeeklyReviewTransaction> = {}): WeeklyReviewTransaction => ({
        ...
        amountCents: -14235, categoryLabel: null, treatment: 'unresolved', bookkeepingNature: null, hasReceipt: false,
    ...
    it('offers unresolved and established expense-direction activity without competing issues', () => {
        ...
        expect(isWeeklyMixedUseCandidate(candidate({ merchant: 'COX COMMUNICATIONS', amountCents: -9680 }))).toBe(true);
    ...
    it('excludes personal, income, structural movements, and competing material issues', () => {
        ...
        expect(isWeeklyMixedUseCandidate(candidate({ amountCents: 250000, treatment: 'business', bookkeepingNature: 'business_income' }))).toBe(false);
    ...
    it('limits the personal-exception sweep to expenses already treated as business', () => {
        ...
        expect(isWeeklyBusinessExpenseSweepCandidate(candidate({ treatment: 'business', bookkeepingNature: 'business_income', amountCents: 10000 }))).toBe(false);
    ...
    it('uses authoritative mixed issue leaves rather than display projection to complete Activity', () => {
        const mixedCompletion = workflowRoute.slice(workflowRoute.indexOf("if(stage==='mixed'&&flowVersion===3)"), workflowRoute.indexOf('for(let index=0', workflowRoute.indexOf("if(stage==='mixed'&&flowVersion===3)")));
