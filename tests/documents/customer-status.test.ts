...
describe('truthful document lifecycle',()=>{
 ...
 it('explains unmatched receipt work without claiming a match',()=>expect(status({...document,state:'completed',document_class:'receipt'})).toBe('Receipt organized — Betti will look for the matching purchase'))
 it('only claims a receipt match with backend evidence',()=>expect(status({...document,state:'completed',document_class:'receipt',receipt_outcome:'matched'})).toBe('Receipt matched'))
