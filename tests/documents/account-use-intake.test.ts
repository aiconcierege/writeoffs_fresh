...
beforeEach(()=>{
 ...
 m.from.mockImplementation((table:string)=>table==='customer_document_dispositions'?{select:()=>({in:async()=>({data:[],error:null})})}:table==='customer_document_scope'?{select:()=>({in:async()=>({data:[{id:'statement',active_transaction_count:24,outside_scope_transaction_count:0}],error:null})})}:table==='bookkeeping_supporting_documents'
