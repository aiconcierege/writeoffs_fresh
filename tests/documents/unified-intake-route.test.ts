...
describe('unified document registration',()=>{
 ...
 it('acknowledges durable registration without waiting for document analysis',async()=>{m.drain.mockImplementation(()=>new Promise(()=>{}));const response=await POST(request());expect(response.status).toBe(200);expect(await response.json()).toEqual({document:{id:body.id},processingPaused:false});expect(m.drain).not.toHaveBeenCalled();expect(response.headers.get('server-timing')).toContain('document_storage_verification');expect(m.after).toHaveBeenCalledTimes(1)})
