-- Specialized percentage questions are material work, alongside canonical review issues.
-- Keep legacy and processed-unmatched receipt rows in the composite work view.
-- Work views and question actions use the same current evidence/age gates.
...
select r.business_id,r.id as record_id,coalesce(f.id,r.id) as transaction_id,r.occurred_on as activity_date,
 ...
 (exists(select 1 from public.list_current_askable_bookkeeping_question_event_ids() q
   join public.bookkeeping_review_events e on e.id=q.event_id
   where e.business_id=r.business_id and e.bookkeeping_record_id=r.id)
   or exists(select 1 from public.current_deduction_attentions a
     where a.business_id=r.business_id and a.bookkeeping_record_id=r.id and a.event_type='opened'
       and d.treatment not in ('personal','excluded'))) as needs_fact,
 exists(select 1 from public.list_current_evidence_question_event_ids() q
   join public.bookkeeping_review_events e on e.id=q.event_id
   where e.business_id=r.business_id and e.bookkeeping_record_id=r.id
     and public.bookkeeping_question_is_historical_documentation(e.reason,e.question_context,r.occurred_on,
       public.bookkeeping_activity_day(r.business_id))) as historical_documentation,
