-- These assessments/facts are scoped by an owned bookkeeping_record_id. Shared
-- business/vehicle/recurring facts retain business-wide invalidation; their derived
-- per-record results must not interrupt already-independent questions.
...
begin
 ...
 if tg_table_name in ('bookkeeping_review_events','bookkeeping_decisions','bookkeeping_documentation_events',
  'bookkeeping_processing_jobs','bookkeeping_mixed_use_answer_provenance',
  'bookkeeping_business_context_assessments','schedule_c_expense_assessments',
  'bookkeeping_meal_substantiation_facts','bookkeeping_tax_treatments','deduction_attention_events') then
