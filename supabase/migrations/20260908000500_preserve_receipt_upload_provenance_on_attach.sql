-- Attaching an existing document is customer-authored linkage, but it must not
-- manufacture customer-upload provenance for system/imported receipts.
...
begin
  ...
  if current_event.id is null then
    insert into public.bookkeeping_receipt_events(business_id,receipt_id,sequence_number,event_type,provenance,actor_user_id,
      context) values(selected_business_id,p_receipt_id,1,'uploaded','system',null,
      jsonb_build_object('schemaVersion',1,'reason','Existing receipt attached without authenticated upload provenance.'))
