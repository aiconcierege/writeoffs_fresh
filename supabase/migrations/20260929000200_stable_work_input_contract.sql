-- Remove experimental wall-clock telemetry from the STABLE result.
-- The canonical snapshot is deterministic within its statement. Timing remains
-- in request diagnostics and separate SQL profiling, never eligibility/version data.
...
create or replace function public.read_betti_work_inputs(p_business_id uuid,p_as_of timestamptz)
returns jsonb language plpgsql stable security invoker set search_path='' as $$
declare
 snapshot_context jsonb; reviews jsonb; askable jsonb; tables jsonb:='{}'; rows jsonb;
 record_ids uuid[]; decision_ids uuid[]; financial_ids uuid[];
begin
 ...
 return jsonb_build_object('version',1,'businessId',p_business_id,'asOf',p_as_of,
  'context',snapshot_context,'reviews',reviews,'askable',askable,'tables',tables);
end;$$;
