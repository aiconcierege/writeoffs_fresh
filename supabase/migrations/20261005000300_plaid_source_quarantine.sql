-- Preserve invalid provider facts without poisoning the Item cursor or inventing money.
alter table public.plaid_transaction_versions add column rejection_reason text,
 add column raw_source jsonb;
alter table public.plaid_transaction_versions add constraint plaid_quarantine_has_no_money
 check (rejection_reason is null or (rejection_reason='INVALID_SOURCE_FACTS' and raw_source is not null
   and jsonb_typeof(raw_source)='object' and amount_cents is null and canonical_financial_transaction_id is null));
comment on column public.plaid_transaction_versions.raw_source is 'Exact parsed provider transaction retained for rejected input; never rendered or logged. Corrected provider revisions supersede it.';
...
begin
  ...
  for event in select value from jsonb_array_elements(p_events) loop
    ...
    if event->>'event_type' <> 'removed'
      and nullif(event->>'rejection_reason','') is null
    ...
    insert into public.plaid_transaction_versions (
      ...
      provider_evidence, canonical_financial_transaction_id, rejection_reason, raw_source, excluded_before_catch_up
    ) values (
      ...
      canonical_transaction_id, nullif(event->>'rejection_reason',''), event->'raw_source',
      event->>'event_type' <> 'removed' and selected_business.catch_up_start_date is not null
        and coalesce((event->>'transaction_date')::date < selected_business.catch_up_start_date,false)
  ...
  return jsonb_build_object('processed', processed_count, 'canonicalized', canonical_count,
    'duplicates', skipped_count, 'quarantined', (select count(*) from public.plaid_transaction_versions v where v.plaid_item_record_id=selected_item.id and v.rejection_reason is not null and not exists(select 1 from public.plaid_transaction_versions s where s.supersedes_version_id=v.id)), 'cursor', p_next_cursor,
    'status', case
      when missing_account_count > 0 then 'needs_attention'
      when selected_item.historical_update_complete then 'connected'
      else 'updating' end);
